...
This setting was created for https://github.com/bailey27/cppcryptfs/issues/62.  Apparently, Google Drive creates hidden desktop.ini files in every directory in the source folder of encrypted files.  These files were preventing users from deleting directories from the un-encrypted side.  If the filesystem is mounted with this setting on, then cppcryptfs will automatically delete unencrypted desktop.ini files when deleting a directory.  
