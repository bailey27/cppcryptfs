...
To mount the filesystem without typing the password, make sure "Save password" is checked, and then either select the path from the path history, in which case the password will be filled in (displaying as dots) in the password field. If you type the path in the path field and press the mount button without typing a password, then if the saved password for that path is found it will be used.
