...
cppcryptfs is compatible with version 2 of the gocryptfs filesystem.  This is the version of the filesystem used by version 1.0 of the gocryptfs software. The only restriction is that only the 128-bit GCM initialization vector length is supported (GCMIV128 in gocrypts.conf).  The legacy  96-bit initialization vector length is not supported.
